---
title: "AE 04: Modelling penguins"
---
...
-   **Demo:** What is the intercept of the regression line?
...
penguins |>
  summarize(
    ...
    sx = sd(flipper_length_mm, na.rm = TRUE),
    b1 = r * sy / sx,
    ybar = mean(body_mass_g, na.rm = TRUE),
    xbar = mean(flipper_length_mm, na.rm = TRUE),
    b0 = ybar - b1 * xbar
  )
```
...
$$
\widehat{body~mass}=-5781+49.6\times flipper~length
$$
...
**Notice!** We got exactly the same numbers for the slope and intercept as we did when we computed them above using $b_1=r\cdot s_y/s_x$ and $b_0=\bar{y}-b_1\bar{x}$. Phew!
