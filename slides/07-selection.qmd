...
format: 
  revealjs: 
    output-file: 07-selection-slides.html
...
## Example: numerical and categorical predictors {.small}
...
- It doesn't matter *technically*. The math doesn't care;
...
- Sometimes the application suggests a natural choice of baseline. You make interpretation and visualization *easier* if you respect that.
...
![](images/07/fit-comparison.png){fig-align="center"}
...
![](images/07/R2.png){fig-align="center"}
...
- We always have $\text{var}(y_i)\geq \text{var}(e_i)$, so $0\leq R^2\leq 1$;
- If $\text{var}(e_i)=0$, then the model explained everything, the fit is perfect (poifect!), and 

$$
R^2=1-\frac{\text{var}(e_i)}{\text{var}(y_i)}=1-\frac{0}{\text{var}(y_i)}=1-0=1.
$$

- If $\text{var}(e_i)=\text{var}(y_i)$, then the model explained nothing, the fit is [perfectly wretched](https://youtu.be/fPntuMTnD8g?si=e_VUyC7WJ-Jv7MVh&t=50), and 

$$
R^2=1-\frac{\text{var}(e_i)}{\text{var}(y_i)}=1-\frac{\text{var}(y_i)}{\text{var}(y_i)}=1-1=0.
$$
...
## Listing out all the possible models {.medium}
...
- In the modern era of "big data," 100 predictors is amateur hour, so literally enumerating the space of models, assigning a score to each, and picking the best becomes computationally difficult;
