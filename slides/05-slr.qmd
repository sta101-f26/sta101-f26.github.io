...
format: 
  revealjs: 
    output-file: 05-slr-slides.html
...
![](images/05/mozarella-ce-phd.png){fig-align="center"}
...
![](images/05/pool-nick-cage.png){fig-align="center"}
...
- On its own, a linear regression model can only summarize association (eg. correlation) among variables. Whether or not it has anything to say about *causation* is a delicate matter, one that we will confront head-on after fall break;
- For now, think like we're still just doing *exploratory* analysis: visualizing and summarizing.
...
Similar story to our other examples:

- budget versus revenue of films;
- weight versus MPG of cars;
- temperature versus ice duration of lakes.

Which variable is *available* first, or easier to measure? In which direction would we typically be *predicting*?
...
# What is this line and how was it drawn?
...
$\mu_{Y \mid X}$ is the *expected value* of $Y$ *given* ("*conditional on*") a particular value of $X$.
...
-   Today, we're studying *simple* linear regression where there is only one predictor. Soon, we will study *multiple* linear regression, where there are \>1 predictors, and each one gets its own coefficient. When you go from 1 predictor to 5 or 10 or 1,000, you run out of letters. So it's easier to add subscripts to the same letter;
...
plot(df, pch = 19, ylim = c(min(df$y), 6), xlim = c(-2, 3))
```

## Best fit line is basically the same either way
...
#abline(rq(y ~ x, data = df), col = "blue", lwd = 2)
legend("bottomright", legend = c("best fit (squared error)", "best fit (absolute error)"), col = c("red", "blue"), lty = 1, lwd = 2, bty = "n")
...
- Minimizing absolute error instead of squared error is like using the *median* instead of the *mean*. It is "more robust" to outliers;
- These options are both "first class citizens;"
- But squared error is more common because it is more mathematically convenient, so that's what we focus on.
...
::: incremental
-   The regression line always goes through the center of mass point ($\bar{x}, \bar{y}$) (ie. the coordinates corresponding to average $X$ and average $Y$):
...
# Recap: the anatomy of simple linear regression

## Notation {.medium}

Imagine a data frame with $n$ rows and 2 numerical columns
...
::: {.column width="65%"}
-   We have $n$ observations (ie. the sample size)
...
-   $i^{th}$ observation ($i$ from $1$ to $n$):
    ...
    -   $\widehat{y_i}=b_0+b_1x_i$ : $i^{th}$ predicted outcome;
...
plot(df, pch = 19, xlim = c(0, 2.5), ylim = c(0, 4), bty = "n", xaxt = "n", yaxt = "n", lwd = 3)
abline(v = 0)
abline(h = 0)
mtext("0", side = 1, at = 0, cex = 2, line = 1)
```
...
plot(df, pch = 19, col = "grey", xlim = c(0, 2.5), ylim = c(0, 4), bty = "n", xaxt = "n", yaxt = "n", lwd = 3)
segments(xi, 0, xi, yi, lty = 2, col = "grey")
segments(0, yi, xi, yi, lty = 2, col = "grey")
points(xi, yi, pch = 19, col = "black", lwd = 3)
mtext(expression(x[1]), side = 1, at = xi, cex = 2, line = 1)
mtext(expression(y[1]), side = 2, at = yi, cex = 2, las = 1)
abline(v = 0)
abline(h = 0)
mtext("0", side = 1, at = 0, cex = 2, line = 1)
```
...
segments(0, yi, xi, yi, lty = 2, col = "grey")
points(xi, yi, pch = 19, col = "black", lwd = 3)
mtext(expression(x[2]), side = 1, at = xi, cex = 2, line = 1)
mtext(expression(y[2]), side = 2, at = yi, cex = 2, las = 1)
abline(v = 0)
abline(h = 0)
mtext("0", side = 1, at = 0, cex = 2, line = 1)
```
...
xi <- df$x[df$y == max(df$y)]
yi <- df$y[df$y == max(df$y)]

plot(df, pch = 19, col = "grey", xlim = c(0, 2.5), ylim = c(0, 4), bty = "n", xaxt = "n", yaxt = "n", lwd = 3)
segments(xi, 0, xi, yi, lty = 2, col = "grey")
segments(0, yi, xi, yi, lty = 2, col = "grey")
points(xi, yi, pch = 19, col = "black", lwd = 3)
mtext(expression(x[i]), side = 1, at = xi, cex = 2, line = 1)
mtext(expression(y[i]), side = 2, at = yi, cex = 2, las = 1)
abline(v = 0)
abline(h = 0)
mtext("0", side = 1, at = 0, cex = 2, line = 1)
```
...
abline(v = 0)
abline(h = 0)
abline(lm(y ~ x, df), col = "red", lwd = 3)
legend(x = 1.9, y = 3.9, c(expression(~b[0]~" + "~b[1]~"x")), text.col = "red", bty = "n", cex = 1.4)
mtext("0", side = 1, at = 0, cex = 2, line = 1)
```
...
legend(x = 1.9, y = 3.9, c(expression(~b[0]~" + "~b[1]~"x")), text.col = "red", bty = "n", cex = 1.4)
segments(0.8, b0 + b1*0.8, 1.8, b0 + b1*0.8, col = "red", lty = 2)
segments(1.8, b0 + b1*0.8, 1.8, b0 + b1*1.8, col = "red", lty = 2)
text(1.3, b0 + b1*0.8 - 0.5, "1", col = "red", cex = 2)
text(1.9, b0 + b1*0.8 + 0.5, expression(b[1]), col = "red", cex = 2)
mtext("0", side = 1, at = 0, cex = 2, line = 1)
```
...
abline(mymodel, col = "red", lwd = 3)
legend(x = 1.9, y = 3.9, c(expression(~b[0]~" + "~b[1]~"x")), text.col = "red", bty = "n", cex = 1.4)
points(0, b0, lwd = 4, col = "red", pch = 19)
mtext(expression(b[0]), side = 2, at = b0, cex = 2, las = 1, line = 0.5, col = "red")
mtext("0", side = 1, at = 0, cex = 2, line = 1)
```
...
abline(mymodel, col = "red", lwd = 3)
legend(x = 1.9, y = 3.9, c(expression(~b[0]~" + "~b[1]~"x")), text.col = "red", bty = "n", cex = 1.4)
mtext(expression(bar(x)), side = 1, at = xbar, cex = 2, line = 1, col = "blue")
mtext(expression(bar(y)), side = 2, at = ybar, cex = 2, col = "blue", las = 1)
segments(xbar, 0, xbar, ybar, lty = 2, col = "blue")
segments(0, ybar, xbar, ybar, lty = 2, col = "blue")
points(xbar, ybar, pch = 19, col = "blue", lwd = 3)
mtext("0", side = 1, at = 0, cex = 2, line = 1)
```
...
abline(mymodel, col = "red", lwd = 3)
legend(x = 1.9, y = 3.9, c(expression(~b[0]~" + "~b[1]~"x")), text.col = "red", bty = "n", cex = 1.4)
mtext(expression(hat(y)[i]), side = 2, at = b0 + b1 * xi, cex = 2, las = 1, col = "red")
segments(0, b0 + b1*xi, xi, b0 + b1*xi, col = "red", lty = 2)
segments(xi, 0, xi, b0 + b1*xi, col = "red", lty = 2)
points(xi, b0 + b1*xi, col = "red", pch = 19, lwd = 3)
mtext("0", side = 1, at = 0, cex = 2, line = 1)
```
...
segments(0, b0 + b1*xi, xi, b0 + b1*xi, col = "red", lty = 2)
#segments(xi, 0, xi, b0 + b1*xi, col = "red", lty = 2)
segments(xi, b0 + b1*xi, xi, yi, lty = 2)
arrows(0.86 * xi, 1.1 * (b0 + b1*xi), 0.86 * xi, 0.95 * yi, code = 3)
text(0.4 * xi, (b0 + b1*xi + yi) / 2, expression(e[i]~" = "~y[i]~" - "~hat(y)[i]), cex = 1.3)
points(xi, b0 + b1*xi, col = "red", pch = 19, lwd = 3)
mtext("0", side = 1, at = 0, cex = 2, line = 1)
```
